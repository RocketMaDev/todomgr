...
int DeleteTodoItem(TodoInfo *g_info, int itemIndex) {
    ...
    g_info->todoCount--;
    g_info->items =
        (TodoItem *)realloc(g_info->items, g_info->todoCount * sizeof(TodoItem));
    return 0;
}
...
int AddTag(TodoInfo *g_info, const char *newTag) {
    ...
    if (newTagList == NULL)
        return -1;
    for (int i = 0; i < g_info->tagCount; i++) newTagList[i] = g_info->tags[i];
...
int DeleteTag(TodoInfo *g_info, int toDeleteTag) {
    ...
    if (remainingTags == NULL)
        return -1;
    for (int i = 0; i < toDeleteTag; i++) remainingTags[i] = g_info->tags[i];
